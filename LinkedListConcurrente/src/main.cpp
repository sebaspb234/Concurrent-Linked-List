...
struct LinkedList
{
    CNode* head, * tail;
    LinkedList();
    bool insert(int x); // Funcion para insertar un nodo
    bool search(int x, CNode*& pred, CNode*& succ); // Funcion para buscar un nodo y apuntar al predecesor y sucesor del nodo
    bool remove(int x); // Funcion para eliminar un nodo
    void print(); // Funcion para imprimir la lista
};

LinkedList::LinkedList()
{
    head = new CNode(-214748368); // Inicializo head con el menor valor que se le puede asignar a un entero para evitar que se inserte o elimine el valor del head en la lista
    tail = new CNode(214748368); // Inicializo tail con el mayor valor que se le puede asignar a un entero para evitar que se inserte o elimine el valode del tail en la lista
    head->next = tail; // Uno el nodo head al nodo tail
}

bool LinkedList::search(int x, CNode*& pred, CNode*& succ)
{
...
{
    cout << "HEAD -> ";
    for (CNode* p = head->next; p && p != tail; p = p->next)
    {
        cout << p->value << " -> ";
    }cout << " NULL" << endl;
}
...
struct Insert {
    ...
    Insert(int min, int max, LinkedList* ptr) :min_(min), max_(max), ptr_(ptr) {}
...
struct Remove {
    ...
    Remove(int min, int max, LinkedList* ptr) :min_(min), max_(max), ptr_(ptr) {}
...
int main()
{
    ...
    for (int i = 0; i < nt; i++) {
        insert_threads.emplace_back(thread(Insert(0, num_operaciones_insertar * nt, &l), num_operaciones_insertar));
        remove_threads.emplace_back(thread(Remove(0, num_operaciones_borrar * nt, &l), num_operaciones_borrar));
    }
    for (int i = 0; i < nt; i++) {
